...
"""
        ...
        playbook_btn = create_styled_button(button_frame, "📚 Attack Chain Planner", self.show_playbook_manager)
    ...
    def show_playbook_manager(self):
        ...
        # FIXED: Added background color to PanedWindow
        manager_pane = tk.PanedWindow(self.main_frame, orient=tk.HORIZONTAL, bg=theme["frame_bg"], sashwidth=8, sashrelief=tk.RAISED)
        ...
        # FIXED: Applied theme colors to the Listbox
        available_tests_list = tk.Listbox(
            left_frame, 
            selectmode=tk.EXTENDED, 
            width=60,
            bg=theme["terminal_bg"], 
            fg=theme["text_fg"],
            selectbackground=theme["accent_fg"],
            selectforeground=theme["terminal_bg"],
            borderwidth=0,
        ...
        add_btn = tk.Button(center_frame, text=">>", command=lambda: self.move_listbox_items(available_tests_list, playbook_steps_list), bg=theme["button_bg"], fg=theme["accent_fg"], width=4)
        add_btn.pack(pady=10, padx=5)
        rem_btn = tk.Button(center_frame, text="<<", command=lambda: self.move_listbox_items(playbook_steps_list, available_tests_list), bg=theme["button_bg"], fg=theme["accent_fg"], width=4)
        rem_btn.pack(pady=10, padx=5)
        manager_pane.add(center_frame, stretch="never")

        # --- Right Pane (Playbook Steps) ---
        ...
        # FIXED: Applied theme colors to the second Listbox
        ...
        # --- Bottom Bar (Controls) ---
        bottom_bar = ttk.Frame(self.main_frame, style='TFrame')
        ...
        def run_playbook():
            ...
            if not steps:
                ...
                return
            playbook_name = playbook_name_entry.get().strip() or "Untitled Playbook"
            self.show_playbook_execution_terminal_view(f"Executing Playbook: {playbook_name}", steps)

        save_btn = create_playbook_button(bottom_bar, text="💾 Save Playbook", command=save_playbook)
        save_btn.pack(side=tk.LEFT, padx=5)
        load_btn = create_playbook_button(bottom_bar, text="📂 Load Playbook", command=load_playbook)
        load_btn.pack(side=tk.LEFT, padx=5)
        run_btn = create_playbook_button(bottom_bar, text="▶️ Run Playbook", command=run_playbook)
        run_btn.pack(side=tk.LEFT, padx=5)
        back_btn = create_playbook_button(bottom_bar, text="🔙 Back to Main Menu", command=self.show_main_menu)
