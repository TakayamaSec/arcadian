...
class BadCipherEDRDemo:
    ...
    def apply_theme(self, theme_name):
        ...
        # Back up content from any active terminal
    ...
    def log_to_terminal(self, message, add_timestamp=True):
        def _log_message():
            try:
                # Find the currently active terminal widget
...
SE EDR demo environment | Managed by Sam Tanner
"""
        ...
        purplesharp_btn = create_styled_button(button_frame, "🟪 PurpleSharp Runner", self.show_ttp_executor_page)
        purplesharp_btn.pack(pady=8)

        playbook_btn = create_styled_button(button_frame, "📚 Playbook Manager", self.show_playbook_manager)
        playbook_btn.pack(pady=8)
        ...
        venom_btn = create_styled_button(button_frame, "🕷️ Venom Spider Attack", self.show_venom_spider_simulation)
    ...
    # --- PURPLESHARP TTP RUNNER MODULE ---
    
    def load_all_tests(self):
        """Loads built-in PurpleSharp tests and custom-defined simulations."""
        # UPDATED: This list now contains the more comprehensive set of tests.
        tests = {
            "Execution": {
    ...
    def show_ttp_executor_page(self):
        ...
        self.clear_frame()
        # BUGFIX: Ensure other terminal widget references are cleared
        self.terminal_output = None 
        
        theme = self.themes[self.current_theme_name]
        title = ttk.Label(self.main_frame, text="🟪 PurpleSharp Individual Test Runner", style='Title.TLabel')
    ...
    def draw_ttp_executor_ui(self, parent):
        ...
        for tactic, techniques in self.all_tests.items():
            tactic_id = self.ttp_tree.insert("", "end", text=tactic, open=True)
            for ttp_id, details in techniques.items():
                self.ttp_tree.insert(tactic_id, "end", text=details['name'], values=(ttp_id,), iid=ttp_id)
        ...
        self.ttp_details_text = scrolledtext.ScrolledText(details_frame, height=8, bg=theme["terminal_bg"], fg=theme["text_fg"], font=(theme["font_family"], 9), relief=tk.FLAT)
        ...
        self.execute_ttp_btn = tk.Button(button_frame, text="💥 Execute Selected Test", command=self.execute_ttp_test, state=tk.DISABLED, bg=theme["button_bg"], fg=theme["accent_fg"])
        self.execute_ttp_btn.pack(side=tk.LEFT, padx=5)
        
        self.cancel_ttp_btn = tk.Button(button_frame, text="🛑 Cancel Test", command=self.cancel_ttp_test, state=tk.DISABLED, bg=theme["button_bg"], fg=theme["accent_fg"])
        self.cancel_ttp_btn.pack(side=tk.LEFT, padx=5)

        self.clear_terminal_btn = tk.Button(button_frame, text="🧹 Clear Terminal", command=self.clear_terminal, state=tk.NORMAL, bg=theme["button_bg"], fg=theme["accent_fg"])
    ...
    def on_ttp_test_select(self, event):
        ...
        if self.ttp_tree.parent(selected_id) and not self.simulation_running:
    ...
    def _execute_test_thread(self, ttp_id):
        ...
        def on_finish():
            self.simulation_running = False
            if hasattr(self, 'execute_ttp_btn') and self.execute_ttp_btn.winfo_exists():
                self.execute_ttp_btn.config(state=tk.NORMAL)
    ...
    # --- CUSTOM SIMULATION AND PLAYBOOK MODULE ---

    def _run_powershell_script(self, script_content, log_prefix):
        if self.stop_operations: return
        self.log_to_terminal(f"--- Running {log_prefix} Script ---", add_timestamp=False)
        had_output = False
        try:
            command_string = f"powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \"{script_content}\""
            process = subprocess.Popen(
                command_string, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True,
    ...
    def _execute_custom_simulation_thread(self, ttp_id):
        ...
        self._run_powershell_script(sim_data.get('simulation_script', '# No simulation script provided'), "Simulation")
        time.sleep(1)
        self._run_powershell_script(sim_data.get('cleanup_script', '# No cleanup script provided'), "Cleanup")
    ...
    def show_simulation_builder(self, sim_id_to_edit=None):
        ...
        script_pane.grid(row=len(fields), column=0, columnspan=2, sticky='nsew', pady=10)
        form_frame.grid_rowconfigure(len(fields), weight=1)
        form_frame.grid_columnconfigure(1, weight=1)

        sim_script_frame = ttk.Frame(script_pane)
        ttk.Label(sim_script_frame, text="Simulation Script (PowerShell)").pack(anchor='w')
        ...
        ttk.Label(clean_script_frame, text="Cleanup Script (PowerShell)").pack(anchor='w')
        ...
        save_btn = tk.Button(button_bar, text="💾 Save Simulation", command=save_simulation, bg=theme["button_bg"], fg=theme["accent_fg"])
        save_btn.pack(side=tk.RIGHT, padx=20)
        back_btn = tk.Button(button_bar, text="🔙 Back to Main Menu", command=self.show_main_menu, bg=theme["button_bg"], fg=theme["accent_fg"])
    ...
    def show_playbook_manager(self):
        ...
        manager_pane = tk.PanedWindow(self.main_frame, orient=tk.HORIZONTAL)
        manager_pane.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        left_frame = ttk.Frame(manager_pane)
        ttk.Label(left_frame, text="Available Tests").pack()
        available_tests_list = tk.Listbox(left_frame, selectmode=tk.EXTENDED, width=60)
        available_tests_list.pack(fill=tk.BOTH, expand=True)
        for tactic, techniques in self.all_tests.items():
            for ttp_id, details in techniques.items():
                available_tests_list.insert(tk.END, f"{ttp_id}: {details['name']}")
        manager_pane.add(left_frame, stretch="always")

        center_frame = ttk.Frame(manager_pane, width=50)
        add_btn = ttk.Button(center_frame, text=">>", command=lambda: self.move_listbox_items(available_tests_list, playbook_steps_list))
        add_btn.pack(pady=10)
        rem_btn = ttk.Button(center_frame, text="<<", command=lambda: self.move_listbox_items(playbook_steps_list, available_tests_list))
        rem_btn.pack(pady=10)
        manager_pane.add(center_frame, stretch="never")

        right_frame = ttk.Frame(manager_pane)
        ...
        playbook_steps_list = tk.Listbox(right_frame)
        ...
        bottom_bar.pack(side=tk.BOTTOM, fill=tk.X, pady=10, padx=10)
        theme = self.themes[self.current_theme_name]
        ...
        def save_playbook():
            ...
            if not name:
                ...
                return
            steps = [item.split(':')[0] for item in playbook_steps_list.get(0, tk.END)]
            playbook_data = {"name": name, "steps": steps}
            file_path = self.playbooks_path / f"{name}.json"
            try:
                with open(file_path, 'w', encoding='utf-8') as f:
        ...
        def run_playbook():
            steps = [item.split(':')[0] for item in playbook_steps_list.get(0, tk.END)]
            if not steps:
                messagebox.showwarning("Warning", "Playbook has no steps to run.")
                return
            self.show_playbook_execution_terminal_view(f"Executing Playbook: {playbook_name_entry.get()}", steps)

        save_btn = tk.Button(bottom_bar, text="💾 Save Playbook", command=save_playbook, bg=theme["button_bg"], fg=theme["accent_fg"])
        save_btn.pack(side=tk.LEFT, padx=5)
        load_btn = tk.Button(bottom_bar, text="📂 Load Playbook", command=load_playbook, bg=theme["button_bg"], fg=theme["accent_fg"])
        load_btn.pack(side=tk.LEFT, padx=5)
        run_btn = tk.Button(bottom_bar, text="▶️ Run Playbook", command=run_playbook, bg=theme["button_bg"], fg=theme["accent_fg"])
        run_btn.pack(side=tk.LEFT, padx=5)
        back_btn = tk.Button(bottom_bar, text="🔙 Back to Main Menu", command=self.show_main_menu, bg=theme["button_bg"], fg=theme["accent_fg"])
        back_btn.pack(side=tk.RIGHT, padx=5)

    def move_listbox_items(self, source_lb, dest_lb):
        selected_indices = source_lb.curselection()
        for i in reversed(selected_indices):
            dest_lb.insert(tk.END, source_lb.get(i))
            source_lb.delete(i)

    def show_playbook_execution_terminal_view(self, title_text, steps):
        self.current_view_factory = lambda: self.show_playbook_execution_terminal_view(title_text, steps)
        self.clear_frame()
        # BUGFIX: Ensure other terminal widget references are cleared
        self.terminal_output = None 
        ...
        back_btn = tk.Button(self.main_frame, text="🔙 Back to Playbook Manager", command=self.show_playbook_manager, bg=theme["button_bg"], fg=theme["accent_fg"])
    ...
    # --- VENOM SPIDER SIMULATION MODULE ---
    ...
    def show_venom_spider_simulation(self):
        ...
        # Restore terminal content if it exists from a previous view
